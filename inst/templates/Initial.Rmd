...
if (!("maf" %in% names(dataForReport))) {
  if ("vc_nonSyn" %in% names(dataForReport)) {
	  dataForReport[["maf"]]=read.maf(maf =dataForReport[["mafFile"]],vc_nonSyn=dataForReport[["vc_nonSyn"]],clinicalData=dataForReport[["clinicalData"]])
  } else {
	  dataForReport[["maf"]]=read.maf(maf =dataForReport[["mafFile"]],clinicalData=dataForReport[["clinicalData"]])
  }
}
...
if (!("performVaf" %in% names(dataForReport)) || dataForReport[["performVaf"]]) { #Not defined or dataForReport[["performVaf"]]==TRUE
  ...
  if (caculateVaf) {
    if (all(c("t_alt_count","t_ref_count") %in% getFields(dataForReport[["maf"]]))) {
      message("Cacualte Vaf based on Alt/(Ref+Alt) count and recored in t_vaf column")
      ...
      warning(paste0("Try to cacualte Vaf based on Alt/(Ref+Alt) count"," but t_alt_count or t_ref_count not found in maf." ))
